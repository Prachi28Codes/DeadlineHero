...
function addToCalendar(title, description, date) {
    const eventTitle = "Study Plan";
const eventDetails = "Generated by Deadline Hero AI";
const startDate = new Date().toISOString().replace(/[-:]|\.\d{3}/g, "");

const url =
`https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(eventTitle)}&details=${encodeURIComponent(eventDetails)}&dates=${startDate}/${startDate}`;
